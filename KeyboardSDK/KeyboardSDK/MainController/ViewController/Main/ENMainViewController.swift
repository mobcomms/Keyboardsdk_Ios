...
public class ENMainViewController: UIViewController, ENViewPrsenter {
    ...
    @IBOutlet weak var viewKeyboardTheme: UIView!
    ...
    public override func viewDidAppear(_ animated: Bool) {
        ...
        if ENSettingManager.shared.isFirstUser {
            // 가이드 화면 이동
            let vc = ENGuideViewController.create()
    ...
    func settingViewMoveGesture() {
        ...
        let viewKeyboardThemeTap = UITapGestureRecognizer(target: self, action: #selector(viewMoveGesture(_:)))
        viewKeyboardTheme.tag = 1
        viewKeyboardTheme.addGestureRecognizer(viewKeyboardThemeTap)
    ...
    @objc func viewMoveGesture(_ sender: UITapGestureRecognizer) {
        if let tag = sender.view?.tag {
            ...
            case 0:
                removeSampleView()
                let vc = ENKeyboardTypeViewController.create()
                vc.delegate = self
...
extension ENMainViewController: ENKeyboardSoundViewControllerDelegate {
    func soundChange() {
        switch ENSettingManager.shared.soundID {
        case 0:
            self.lblKeyboardSound.text = "기본"
...
extension ENMainViewController: ENKeyboardTypeChangeDelegate {
