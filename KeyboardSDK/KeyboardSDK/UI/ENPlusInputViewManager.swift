...
//  KeyboardSDK
//
//  Created by cashwalkKeyboard on 2021/07/26.
//
...
public class ENPlusInputViewManager {
    ...
    var enKeyboardMoreView: ENKeyboardMoreView? = nil
    ...
    public func viewWillLayoutSubviews() {
        ...
        if isPortrait {
            ...
            if let settingView = enKeyboardMoreView {
            ...
            if let data = data, let jsonString = String(data: data, encoding: .utf8) {
                ...
                if let jsonData = jsonString.data(using: .utf8) {
                    do {
                        ...
                        if data.Result == "true" {
                            DispatchQueue.main.async {
                                ENSettingManager.shared.readyForPoint = data.total_point
                            ...
                            DispatchQueue.main.async {
                                ENSettingManager.shared.readyForPoint = 0
                                if let custom = self.customAreaView {
                                    custom.updateRewardPointToolbar()
                                }
                            }
...
extension ENPlusInputViewManager: ENKeyboardCustomAreaViewDelegate {
    ...
    public func enKeyboardCustomAreaView(_ customAreaView:ENKeyboardCustomAreaView, targetButton: UIButton, moveApp completion: (() -> Void)?) {
        if targetButton.isSelected {
            ...
            let enContentView = ENContentView()
            
            enContentView.delegate = self
            keyboardViewManager?.showCustomContents(customView: enContentView)
    ...
    public func enKeyboardCustomAreaView(_ customAreaView:ENKeyboardCustomAreaView, targetButton: UIButton, movePPZone completion: (() -> Void)?) {
        inputViewController?.open(url: URL(string: ENKeyboardSDKSchemeManager.ppzone)!)
    }
    
    public func enKeyboardCustomAreaView(_ customAreaView:ENKeyboardCustomAreaView, targetButton: UIButton, moveCoupang completion: (() -> Void)?) {
    ...
    public func enKeyboardCustomAreaView(_ customAreaView:ENKeyboardCustomAreaView, targetButton: UIButton, movePointList completion: (() -> Void)?) {
        if targetButton.isSelected {
            self.keyboardViewManager?.restoreKeyboardViewFrom(customView: nil)
            ...
            let enPointContentView = ENPointContentView()
            enPointContentView.delegate = self
            keyboardViewManager?.showCustomContents(customView: enPointContentView)
    ...
    public func enKeyboardCustomAreaView(_ customAreaView:ENKeyboardCustomAreaView, targetButton: UIButton, moveSetting completion: (() -> Void)?) {
        if targetButton.isSelected {
            self.keyboardViewManager?.restoreKeyboardViewFrom(customView: nil)
            ...
            enKeyboardMoreView = ENKeyboardMoreView()
            enKeyboardMoreView?.delegate = self
            
            keyboardViewManager?.showCustomContents(customView: enKeyboardMoreView)
            
            targetButton.isSelected = true
            targetButton.backgroundColor = keyboardTheme?.themeColors.tab_off
        }
    }
    public func enKeyboardCustomAreaView(_ customAreaView:ENKeyboardCustomAreaView, targetButton: UIButton, moveCashDeal completion: (() -> Void)?) {
...
extension ENPlusInputViewManager: ENNotifyViewDelegate, ENKeyboardMoreViewDelegate {
    ...
    func openTheme() {
        inputViewController?.open(url: URL(string: ENKeyboardSDKSchemeManager.theme)!)
    }
    ...
    func openClipBoard() {
        ...
        if let customAreaView = customAreaView {
            ...
            let clipboardView = ENClipboardView()
    ...
    func openSetting() {
        inputViewController?.open(url: URL(string: ENKeyboardSDKSchemeManager.keyboardSetting)!)
    }
    func openCashDeal() {
        inputViewController?.open(url: URL(string: ENKeyboardSDKSchemeManager.keyboardCashDeal)!)
    }
    func openInquiry() {
        inputViewController?.open(url: URL(string: ENKeyboardSDKSchemeManager.keyboardInquiry)!)
    }
    func openCash(){
        if let customAreaView = customAreaView {
            self.resetKeyboard(customAreaView)
...
extension ENPlusInputViewManager:  ENPointContentViewDelegate, ENClipboardViewDelegate,ENContentViewDelegate {
