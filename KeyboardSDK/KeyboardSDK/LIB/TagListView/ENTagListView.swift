...
@objc public protocol ENTagListViewDelegate {
    @objc optional func tagPressed(_ title: String, tagView: ENTagView, sender: ENTagListView) -> Void
    @objc optional func tagRemoveButtonPressed(_ title: String, tagView: ENTagView, sender: ENTagListView) -> Void
}

@IBDesignable
open class ENTagListView: UIView {
    ...
    @IBOutlet open weak var delegate: ENTagListViewDelegate?
    
    open private(set) var tagViews: [ENTagView] = []
    ...
    open override func prepareForInterfaceBuilder() {
        addTag("Welcome")
        addTag("to")
        addTag("ENTagListView").isSelected = true
    }
    ...
    private func createNewTagView(_ title: String) -> ENTagView {
        let tagView = ENTagView(title: title)
    ...
    @discardableResult
    open func addTag(_ title: String) -> ENTagView {
        defer { rearrangeViews() }
        return addTagView(createNewTagView(title))
    }
    
    @discardableResult
    open func addTags(_ titles: [String]) -> [ENTagView] {
        return addTagViews(titles.map(createNewTagView))
    }
    
    @discardableResult
    open func addTagView(_ tagView: ENTagView) -> ENTagView {
        defer { rearrangeViews() }
    ...
    @discardableResult
    open func addTagViews(_ tagViewList: [ENTagView]) -> [ENTagView] {
        defer { rearrangeViews() }
        tagViewList.forEach {
            tagViews.append($0)
    ...
    @discardableResult
    open func insertTag(_ title: String, at index: Int) -> ENTagView {
        return insertTagView(createNewTagView(title), at: index)
    }
    

    @discardableResult
    open func insertTagView(_ tagView: ENTagView, at index: Int) -> ENTagView {
        defer { rearrangeViews() }
    ...
    open func removeTagView(_ tagView: ENTagView) {
        defer { rearrangeViews() }
    ...
    open func selectedTags() -> [ENTagView] {
        return tagViews.filter { $0.isSelected }
    }
    
    // MARK: - Events
    
    @objc func tagPressed(_ sender: ENTagView!) {
        sender.onTap?(sender)
        delegate?.tagPressed?(sender.currentTitle ?? "", tagView: sender, sender: self)
    }
    
    @objc func removeButtonPressed(_ closeButton: ENCloseButton!) {
